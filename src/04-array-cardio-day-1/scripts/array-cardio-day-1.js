(function (global, docRef) {
  ...
  function runInventorsBornIn1500s() {
    const inventorsBornIn1500s = (getInventorData() ?? []).filter((inventor) => {
  ...
  function runInventorFirstAndLastNames() {
    const inventorFirstLastNames = (getInventorData() ?? []).map((inventor) => {
  ...
  function runInventorsByBirthdate() {
    const inventorsByBirthdate = (getInventorData() ?? []).sort((a, b) => {
      if (a.year > b.year) {
        return 1;
      }
      if (a.year < b.year) {
  ...
  function runInventorsTotalYearsAlive() {
    const totalYearsAlive = (getInventorData() ?? []).reduce((acc, curr) => {
  ...
  function runInventorsSortedByYearsLived(asc = true) {
    const inventorsSorted = (getInventorData() ?? []).sort((a, b) => {
  ...
  function runGetParisBoulevardsWithde() {
    const boulevardsWithDe = (getParisBoulevardData() ?? []).filter((boulevard) => {
  ...
  function runGetPeopleSortedByLastName(asc = true) {
    const peopleSortedByLastName = (getPeopleData() ?? []).sort((a, b) => {
  ...
  function runSumOfDataInstances() {
    const instanceSum = (getData() ?? []).reduce((acc, curr) => {
